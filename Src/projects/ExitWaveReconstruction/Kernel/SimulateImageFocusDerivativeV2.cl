/**
 * \brief OpenCL kernel for TEM "image" simulation using the focus derivative of the full TCC
 * 
 * Each work item computes exactly one pixel (u, v) of the simulated
 * image, where u = get_global_id ( 0 ) and v = get_global_id ( 1 ).
 * 
 * This kernel is equivalent to SimulateImageFocusDerivativeV1, with the
 * exception that buffer objects instead of images are used to
 * transmit data between the host program and the GPU. Thus this
 * kernel can also be run on GPUs without OpenCL image support
 * and / or with double precision (if the GPU supports the double
 * datatype).
 * 
 * 
 ...
 *   - WAVELENGTH_SQR (squared electron wavelength in nm^2)
 *   - WAVELENGTH_CUB (cubed electron wavelength in nm^3)
 *   - WAVELENGTH_QRT (electron wavelength to the fourth power in nm^4)
 ...
 *   - PI (approximation of pi of type RealType)
 */
kernel void SimulateImageFocusDerivativeV2 ( global RealType4 *Input,
  ...
  for ( int y = y_start; y < y_end ; ++y, freqY += (RealType2)( INV_LEN_Y, INV_LEN_Y ) ) {
    ...
    for ( int x = x_start; x < x_end ; ++x, freqX += (RealType2)( INV_LEN_X, INV_LEN_X ) ) {
      ...
      // Compute product of the exit wave entries
      exit_wave_product = (RealType2)( p1.s0 * p2.s0 + p1.s1 * p2.s1, -p1.s0 * p2.s1 + p1.s1 * p2.s0 );
      
      // Compute the complete transfer function
      const RealType freq2_normsqr = freqX.s0 * freqX.s0 + freqY.s0 * freqY.s0;
      const RealType freq1_normsqr = freqX.s1 * freqX.s1 + freqY.s1 * freqY.s1;
      const RealType freq_dot = freqX.s0 * freqX.s1 + freqY.s0 * freqY.s1;
      
      RealType2 diff_coeff = (RealType2)( focus * WAVELENGTH_SQR * ( freq1_normsqr + freq2_normsqr - 2 * freq_dot ), -PI * WAVELENGTH * ( freq1_normsqr - freq2_normsqr ) );
      diff_coeff.s0 += WAVELENGTH_QRT * SPHERICAL_ABERRATION * ( freq1_normsqr * freq1_normsqr + freq2_normsqr * freq2_normsqr - freq_dot * ( freq1_normsqr + freq2_normsqr ) );
      diff_coeff.s0 *= 2 * SPATIAL_COHERENCE_COEFF;
      
      transfer_function.s0 = pure_phase_transfer.s0 * diff_coeff.s0 - pure_phase_transfer.s1 * diff_coeff.s1;
      transfer_function.s1 = pure_phase_transfer.s0 * diff_coeff.s1 + pure_phase_transfer.s1 * diff_coeff.s0;
      
      transfer_function *= spatial_coh * temporal_coh;
