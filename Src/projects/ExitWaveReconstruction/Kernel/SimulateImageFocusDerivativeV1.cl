 ...
 * float. See SimulateImageFocusDerivativeV2 for GPUs without OpenCL
 * image support or if double precision is needed.
 * 
 * 
 ...
 *   - PI (floating point approximation of pi)
 */
constant sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;

kernel void SimulateImageFocusDerivativeV1 ( read_only image2d_t Input,
  ...
  for ( int y = y_start; y < y_end ; ++y, freqY += (float2)( INV_LEN_Y, INV_LEN_Y ) ) {
    ...
    for ( int x = x_start; x < x_end ; ++x, freqX += (float2)( INV_LEN_X, INV_LEN_X ) ) {
      ...
      const float freq2_normsqr = freqX.s0 * freqX.s0 + freqY.s0 * freqY.s0;
      const float freq1_normsqr = freqX.s1 * freqX.s1 + freqY.s1 * freqY.s1;
      const float freq_dot = freqX.s0 * freqX.s1 + freqY.s0 * freqY.s1;
      
      float2 diff_coeff = (float2)( focus * WAVELENGTH_SQR * ( freq1_normsqr + freq2_normsqr - 2 * freq_dot ), -PI * WAVELENGTH * ( freq1_normsqr - freq2_normsqr ) );
